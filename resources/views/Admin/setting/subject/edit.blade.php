    ...
    <form action="{{route('subjects.update',$subject->id)}}" method="post">
        ...
        <div>
            ...
            @foreach (config('translatable.locales') as $locale)
                ...
                <input class="form-control" name="{{$locale}}[name]" value="{{$subject->translateOrNew($locale)->name, true}}" type="text">
            ...
            @else
            <input name="{{Config::get('app.locale')}}[name]"  type="text" value="{{$subject->name}}">
